...
The function of this code is to display a window and let user either report an item or find an item"""
...
def launch_main_app():
  ...
  # This code ise for the menu and was used via the help of the internet and also the code is used to create a menu bar with options such as view reported items, view found items, exit and about. 
    ...
    picture.image = image  # keep a reference so it isn't garbage collected
...
#this makes it so that this file is only run if I run it directly and not import it from another file.
